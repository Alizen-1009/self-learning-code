...
- **命名对照**（三方易撞，固定如下）：T×T 耦合矩阵 = `C`（博客 `A_{s,s'}`）· 衰减门 = `a`（博客 `A`/`e^g`）· buffer 容量 = `L`（不动）· 第 5 课 Amdahl 符号 = `f` / `x` / `E`（已 grep 确认未撞名；`s` 被第 3 课投机下标占用，故局部加速用 `x` 不用 `s`）· prefix cache 块粒度 = `P`（2026-08-12 加入第 4 课第三侧，已 grep 确认全工作区未占用；只在那一节出现，第 4 课符号预算因此 10 → 11）。
...
| 4 | 三个不变量（`0004-cache-flush-invariants.html`） | 能凭记忆画出状态机、说出三条不变量「破了会怎样」、从**三侧**夹出 `L`。已补 `L* = √(2dn/(d+n))` 与第三侧（块粒度 `P` 的天花板，有效周期 = `min(L−1, P)`） | 已交付 2026-08-11 · 第三侧补于 2026-08-12 |

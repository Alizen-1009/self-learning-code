...
- ✅ 第 2 课：`ldmatrix` → `mma.sync` fragment 映射与写回审查（`lessons/0002`）
- ✅ 新增知识源：`modern-gpu-programming-for-mlsys`（Blackwell 主脊 + TIRx 透镜，`learning-records/0002`）
- 📖 当前第 3 课：把 CuTe Layout 当作坐标函数，手算连续性、padding 与 alias（`lessons/0003`）
- ⏭ 后续：hierarchical Layout 与 composition/coalesce/divide/product

...
- 第 1～2 课已覆盖从 WMMA 到裸 `ldmatrix` / `mma.sync` fragment 映射与写回审查。
- 当前第 3 课：`lessons/0003-cute-layout-as-coordinate-function.html`，从坐标函数进入 CuTe/CuTe DSL。
- Layout 速查已建立：`reference/cute-layout-reading.html`；总术语表见 `reference/glossary.html`。
- 后续方向：hierarchical Layout 与 composition/coalesce/divide/product，再进入 Copy Atom / TiledCopy。
